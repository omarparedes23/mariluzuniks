...
import { updateCompra } from '@/lib/actions/purchases'
import type { Compra, Producto } from '@/types/admin'
...
export default function EditCompraForm({ compra, products }: { compra: Compra; products: Producto[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await updateCompra(compra.id, {
    ...
    if (result.success) {
      router.push(`/admin/compras/${compra.id}`)
      router.refresh()
    } else {
      setError(result.error || 'Error al actualizar la compra')
      setLoading(false)
    }
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        <Link href={`/admin/compras/${compra.id}`} className="p-2 text-muted hover:text-cream transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="font-serif text-3xl text-cream">Editar Compra</h1>
          <p className="text-muted text-sm bg-yellow-500/10 text-yellow-400 px-3 py-1 rounded-full inline-block mt-1">
            Al guardar se recalcula el stock de todos los productos
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full bg-bg border border-gold/30 rounded px-4 py-3 text-cream focus:border-gold focus:outline-none transition-colors"
              >
                {tiposComprobante.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href={`/admin/compras/${compra.id}`} className="text-muted hover:text-cream transition-colors">
            Cancelar
          </Link>
        </div>
      </form>
